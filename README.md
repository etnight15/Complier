# Лабораторная работа №1

## Название и цель лабораторной работы
Название: Разработка пользовательского интерфейса (GUI) для языкового процессора.
Цель: Создание кроссплатформенного графического интерфейса (GUI) для языкового процессора в виде специализированного текстового редактора.

## Сведения об авторе
Марков Д.Д., АВТ-314

## Описание проекта
Оконное приложение представляет собой текстовый редактор с графическим интерфейсом, реализующий базовые операции работы с текстовыми файлами. Включает главное меню, панель инструментов, область редактирования и область вывода результатов. Приложение разработано с учетом дальнейшего расширения функционала для создания языкового процессора.

## Используемые технологии
Python 3.9+, PyQt6
Среда разработки: VS Code

## Инструкция по сборке и запуску
### Установка зависимостей
```bash
# Создание виртуального окружения (рекомендуется)
python -m venv venv

# Активация виртуального окружения
# Windows:
venv\Scripts\activate
# Linux/Mac:
source venv/bin/activate

# Установка зависимостей
pip install -r requirements.txt
```
## Руководство пользователя
Оконное приложение предстваляет собой текстовый редактор, на главном окне программы можно видеть в верхней части расположнна рабочая область и панель инструментов, а в нижней части - вывод результатов.
<img width="1626" height="891" alt="image" src="https://github.com/user-attachments/assets/2bdff32b-d552-4bfe-b659-ade1ade94bf4" />
<img width="1305" height="699" alt="image" src="https://github.com/user-attachments/assets/b7d27e13-e040-4040-b948-9c067f592b38" />

Выше панели инструментов расположено меню:
1. Работа с файломи
<img width="213" height="203" alt="image" src="https://github.com/user-attachments/assets/2922adab-ed74-4e7c-8c3e-c6b0de30272b" />

2. Правка
<img width="174" height="264" alt="image" src="https://github.com/user-attachments/assets/f8c79ef6-db07-4590-be77-0506b29a2e70" />

3. Кнопка пуск
<img width="188" height="72" alt="image" src="https://github.com/user-attachments/assets/c936f0e1-3c95-49d2-9d63-fdde07296640" />

4. Справка о программе, а также содержащяя информацию о горячих клавишах
<img width="152" height="102" alt="image" src="https://github.com/user-attachments/assets/f0cc7206-f64f-406d-acf7-3f5745c81fc3" />

### Горячие клавиши:
<img width="470" height="318" alt="image" src="https://github.com/user-attachments/assets/6fd56853-a7d2-4871-8ee6-1aae9834a2dd" />


# Лабораторная работа №2

## Название и цель лабораторной работы
Название: Разработка лексического анализатора (сканера)
Цель работы: Изучить назначение и принципы работы лексического анализатора в структуре компилятора. Спроектировать алгоритм (диаграмму состояний) и выполнить программную реализацию сканера для выделения лексем из входного текста. Интегрировать разработанный модуль в ранее созданный графический интерфейс языкового процессора.

## Постановка задачи
Разработать лексический анализатор (сканер) в соответствии с индивидуальным вариантом задания, интегрировать его в приложение из лабораторной работы №1 и обеспечить наглядный вывод результатов.

Требования к реализации сканера:
1. Спроектировать диаграмму состояний конечного автомата, реализующего сканер, согласно варианту задания.
2. Разработать программный модуль лексического анализа, который:
   - принимает на вход строку (исходный текст программы);
   - выделяет все допустимые лексемы согласно варианту;
   - классифицирует лексемы по типам (например: "ключевое слово", "идентификатор", "число", "оператор", "разделитель").
   - любые символы, не соответствующие ни одному из допустимых типов лексем, считать недопустимыми и выводить сообщение об ошибке с указанием позиции.
   - учитывает многострочность входного текста.

## Вариант задания
55. Объявление вещественной константы с инициализацией на языке Pascal
Примеры корректных входных строк:
- const pi: real = 3.14;
- const x: real = 0.5;
- const myConst: real = 10.0;

## Диаграмма состояний
<img width="789" height="766" alt="image" src="https://github.com/user-attachments/assets/9037db67-9011-46a7-953f-7b164e8afb75" />

## Тестовые примеры
1. Корректная строка: const pi: real = 3.14;
<img width="1349" height="464" alt="image" src="https://github.com/user-attachments/assets/dc29908c-d937-40ec-af8c-66233aacc621" />

2. Строка с недопустимым символом: const pi: real = 3.14@
<img width="1919" height="462" alt="image" src="https://github.com/user-attachments/assets/3da6d038-c1f1-4194-8fda-c30f1260786b" />

3. Многострочный пример:
const x: real = 0.5;
const myConst: real = 10.0;
<img width="1917" height="795" alt="image" src="https://github.com/user-attachments/assets/3f60f408-37ff-4714-aea2-c58b1344a75e" />
<img width="1919" height="124" alt="image" src="https://github.com/user-attachments/assets/a6b45cc1-2c4c-4e71-9d89-90ef2e458b75" />


# Лабораторная работа №3

## Название и цель лабораторной работы
Название: Разработка синтаксического анализатора (парсера)
Цель: Изучить назначение и принципы работы синтаксического анализатора в структуре компилятора. Спроектировать грамматику, построить соответствующую схему метода анализа грамматики и выполнить программную реализацию парсера с нейтрализацией синтаксических ошибок методом Айронса. Интегрировать разработанный модуль в ранее созданный графический интерфейс языкового процессора.

## Постановка задачи
Разработать синтаксический анализатор (парсер) в соответствии с индивидуальным вариантом курсовой (расчетно-графической) работы, интегрировать его в приложение из лабораторной работы №1 и обеспечить наглядный вывод результатов анализа.

Требования к разработке парсера:
1. Разработать грамматику для заданной синтаксической конструкции.
2. Построить схему метода анализа на основе разработанной грамматики.
3. Выполнить программную реализацию алгоритма работы синтаксического анализа.
4. Реализовать алгоритм нейтрализации синтаксических ошибок методом Айронса.
5. Входные данные — строка (текст программного кода) из области редактирования.
6. Выходные данные: При успешном анализе корректной строки — сообщение об отсутствии ошибок; при обнаружении ошибок — таблица с описанием каждой ошибки.

## Вариант задания
Объявление вещественной константы с инициализацией на
языке Pascal

## Примеры корректных входных строк:
- const pi: real = 3.14;
- const x: real = 0.5;
- const myConst: real = 10.0;

## Разработка грамматики
<img width="783" height="387" alt="image" src="https://github.com/user-attachments/assets/c43009f9-0ccb-41bb-ad09-30b0513aae73" />

## Классификация грамматики
Грамматика является автоматной:
<img width="361" height="34" alt="image" src="https://github.com/user-attachments/assets/322ec8b2-3de8-4890-bd27-18a228ad22d2" />

Все продукции имеют вид:
<img width="532" height="110" alt="image" src="https://github.com/user-attachments/assets/de95f978-6304-47ca-a4b3-592b3d0c255b" />

## Метод анализа
<img width="975" height="761" alt="image" src="https://github.com/user-attachments/assets/c7898e74-f4a6-4ba9-ba0a-95c625fff6a4" />

## Диагностика и нейтрализация синтаксических ошибок
Метод Айронаса явлетеся алгоритмом восстановления после синтаксических ошибок, который позволяет продолжить анализ программы даже при обнаружении ошибки. В моем синтаксическом анализаторе он разработан через механизм пропуска ошибочных токенов до встречи с ожидаемыми символами. При обнаружении ошибки анализатор фиксирует ошибку и вызывает функцию _skip_until(), которая последовательно удаляет из входного потока все токены, не входящие в множество допустимых символов для текущего этапа. Как только встречается ожидаемый токен, разбор продолжается.

Пример:
<img width="1919" height="352" alt="image" src="https://github.com/user-attachments/assets/456f643d-b96e-4298-ba78-659c07f85c4c" />
<img width="1917" height="398" alt="image" src="https://github.com/user-attachments/assets/f5e4e500-56f6-4f5e-889d-0fe67ac5c325" />

В данном примере анализатор работает следующим образом: Проверят первый токен и фиксирует ошибку, далее вызывается функция и пропускает токен real, продолжается анализ и в конце строки проверяет ";" и фиксирует вторую ошибку.

## Тестовые примеры
1. Корректный пример: const pi: real = 3.14;
<img width="911" height="582" alt="image" src="https://github.com/user-attachments/assets/c2c92357-4371-4da7-af66-631940950434" />

2. Неверный пример: const pi: real = .14;
<img width="1622" height="451" alt="image" src="https://github.com/user-attachments/assets/9d8487c9-62c3-4644-9461-0ed6bd3c090d" />

3. Неверный пример: const pi:: = 3.14 FDFSF
<img width="1919" height="664" alt="image" src="https://github.com/user-attachments/assets/32aaf4c7-0be8-4056-bb73-dd06ebd05aeb" />


# Лабораторная работа №4

## Название и цель лабораторной работы
Название: Реализация алгоритма поиска подстрок с помощью регулярных выражений
Цель: Изучить теоретические основы регулярных выражений и их применение для поиска и извлечения подстрок из текста. Освоить практические навыки использования библиотечных средств работы с регулярными выражениями, а также интеграцию алгоритмов поиска в графический интерфейс приложения.

## Постановка задачи
Разработать модуль поиска подстрок с использованием регулярных выражений, интегрировать его в существующее приложение (текстовый редактор) и обеспечить наглядный вывод результатов.

## Вариант задания
1. Построить РВ для валидации французских номеров телефонов.
2. Построить РВ для поиска переменной в стиле snakeCase (используют только строчные буквы и нижние подчеркивания).
3. Построить РВ для проверки надежности пароля. Требования к надежности пароля: Имеет длину не менее 14 символов. Хотя бы одна заглавная русская буква. Хотя бы одна строчная русская буква. Хотя бы одна цифра. Хотя бы один специальный символ из списка: ()#?!|/@/$%\^&*-_.

## Регулярные выражения
1. 
a. Построить РВ для валидации французских номеров телефонов.

b. <img width="483" height="364" alt="image" src="https://github.com/user-attachments/assets/809b8e11-bf38-4554-b419-96b31ab572ac" />

c. Примеры строк, которые должны находиться: 0123456789, 01 23 45 67 89, 01-23-45-67-89

d. Примеры строк, которые не должны находиться: 0012345678, 1234567890, 01234567890

e. Тестовые примеры
<img width="1919" height="416" alt="image" src="https://github.com/user-attachments/assets/e47d6be1-f25c-4e52-a13b-13d06b010ad6" />


2.
a. Построить РВ для поиска переменной в стиле snakeCase (используют только строчные буквы и нижние подчеркивания).

b. <img width="802" height="187" alt="image" src="https://github.com/user-attachments/assets/bcdfac26-2b15-4e57-818f-5fad3231a612" />
 
c. Примеры строк, которые должны находиться: hello_world, one_two_three

d. Примеры строк, которые не должны находиться: username, User_name

e. Тестовые примеры
<img width="1919" height="364" alt="image" src="https://github.com/user-attachments/assets/51894531-f2dd-411b-b1ac-d6df04cba19f" />

3.
a. Построить РВ для проверки надежности пароля. Требования к надежности пароля: Имеет длину не менее 14 символов. Хотя бы одна заглавная русская буква. Хотя бы одна строчная русская буква. Хотя бы одна цифра. Хотя бы один специальный символ из списка: ()#?!|/@/$%\^&*-_.

b. <img width="981" height="205" alt="image" src="https://github.com/user-attachments/assets/5739c601-d0f6-4ab7-b9d6-44436f527f85" />

c. Примеры строк, которые должны находиться: ПриветМир123456!

d. Примеры строк, которые не должны находиться: приветмир123456!, ПРИВЕТМИР123456!

e. Тестовые примеры
<img width="1919" height="344" alt="image" src="https://github.com/user-attachments/assets/9d8351f2-b082-41b9-bb0b-724b87bbc51f" />

## Дополнительное задание
Необходимо построить автомат по регулярному выражению.

<img width="931" height="487" alt="image" src="https://github.com/user-attachments/assets/fa799de6-c6e5-430a-a8fd-0d906a0a2dd4" />

# Лабораторная работа №5

## Название работы
Построение AST и проверка контекстно-зависимых условий

## Цель работы
Изучить назначение и принципы работы семантического анализатора в структуре компилятора. Освоить методы построения абстрактного синтаксического дерева (AST) и проверки контекстно-зависимых условий (семантических правил) для заданной синтаксической конструкции.

## Вариант задания
Объявление вещественной константы с инициализацией на языке Pascal

## Примеры верных строк
```
const pi: real = 3.14;
const x: real = 0.5;
const myConst: real = 10.0;
```

## Контекстно-зависимые условия
| № | Проверка | Пример | Ожидаемое сообщение |
|---|----------|--------|----------------------|
| 1 | Уникальность имён | `const a: real = 1.0;`<br>`const a: real = 2.0;` | Повторное объявление идентификатора 'a' |
| 2 | Совместимость типа и инициализатора | `const x: real = y;` | Несовместимость типов: объявлен 'real', а получен '<тип>' |
| 3 | Использование только объявленных идентификаторов | `const b: real = a;` | Идентификатор 'a' не объявлен |
| 4 | Корректный пример (без ошибок) | `const pi: real = 3.14;` | Семантических ошибок не обнаружено. |

## Структура AST

### Описание типов узлов
| Тип | Назначение |
|-----------|------------|
| program | Корень программы: список объявлений констант. Дочерние узлы — const_declaration. |
| const_declaration | Объявление вида const имя : тип = значение ;. |
| keyword | Ключевое слово const. |
| identifier | Имя константы (pi). |
| type | Имя типа данных (real). |
| value | Вещественный литерал инициализатора (3.14). |

### Рисунок AST для верной строки
<img width="686" height="769" alt="image" src="https://github.com/user-attachments/assets/ab649972-374c-4439-b1d5-6d2c740acc1f" />

### Формат вывода AST в программе
<img width="464" height="201" alt="image" src="https://github.com/user-attachments/assets/a81d3ec8-9bd7-40b0-9000-7ef0c38c0805" />

## Тестовые примеры (скриншоты)
Пример 1 — без семантических ошибок, корректное дерево
<img width="1919" height="566" alt="image" src="https://github.com/user-attachments/assets/b3e7f559-df8b-40a2-b3e4-c62563a08b72" />

Пример 2 — дублирующее объявление
<img width="1338" height="1010" alt="image" src="https://github.com/user-attachments/assets/fd949207-103f-422d-aa25-e1e620b6b6f8" />

Пример 3 - Недопустимое число
<img width="1919" height="355" alt="image" src="https://github.com/user-attachments/assets/d420123e-60ea-43b4-8c09-00f916d51d15" />


## Инструкция по запуску           
1. Установить Python 3.10+              
2. Создание виртуального окружения
```
python -m venv .venv
.venv\Scripts\activate
```
3. Установка PyQt6
```
python -m pip install PyQt6
```
4. Для запуска нужно перейти в каталог в каталог с main.py и выполнить
```
python main.py
```
5. В текством редакторе можно вводить текст и запускать нажимая кнопку "Пуск" (F5)/.

# Лабораторная работа №6

## Название
Создание внутренней формы представления программы

## Вариант задания
Грамматика на языке Pascal Язык программирования: Pascal

Полное определение грамматики:
```
E → TA
A → ε | + TA | - TA
T → FB
B → ε | * FB | / FB
F → num | id | (E)
id → letter {letter | digit}
num → digit {digit}
```

Примеры верных строк:
```
1 + 2 * 3
(1 + 2) * 3
a+1
(10 + 5) * 2 - 3
```
## Лексические и синтаксические ошибки
### Диаграмма лексера:

<img width="421" height="1461" alt="Диаграмма без названия drawio (5)" src="https://github.com/user-attachments/assets/494e9ca9-1e13-43a9-b9f1-41baaf1c9de1" />

### Схема рекурсивного спуска:

<img width="732" height="615" alt="image" src="https://github.com/user-attachments/assets/d53663dc-0de4-43d4-8efe-05bc37fa2c0a" />

### Тестовые примеры:

<img width="1241" height="805" alt="image" src="https://github.com/user-attachments/assets/9e768a38-1a3d-4a5e-817a-c9e66efb6284" />

<img width="1240" height="809" alt="image" src="https://github.com/user-attachments/assets/7a740793-6092-4a09-b0eb-5452b0cc3b7e" />

<img width="1239" height="803" alt="image" src="https://github.com/user-attachments/assets/7c0427c5-58ce-4266-ba8a-234a3c04d43e" />

## Внутренняя форма представления программы (тетрады)
<img width="1279" height="180" alt="image" src="https://github.com/user-attachments/assets/07434f04-0857-4e3f-a941-740b2f952360" />

## ПОЛИЗ
<img width="1282" height="119" alt="image" src="https://github.com/user-attachments/assets/2b2a7f35-e75f-47aa-af36-a7ea7111e7f6" />

# Лабораторная работа №7
## Название 
Преобразование и анализ кода с использованием Clang и LLVM

## Цель работы
Познакомиться с инструментами Clang и LLVM, научиться получать абстрактное синтаксическое дерево (AST) и промежуточное представление (LLVM IR) для кода на C/C++, применять базовые оптимизации, строить граф потока управления (CFG), а также проанализировать влияние оптимизаций на синтаксические конструкции языка.

## Постановка задачи
1. Установить Clang, LLVM, opt и Graphviz.
2. Скомпилировать простой C-файл и получить AST и LLVM IR.
3. Использовать opt для оптимизации IR.
4. Построить CFG до и после оптимизаций.
5. Выполнить индивидуальное задание по варианту.

## Установка и подготовка среды
Команда установки:
```
sudo apt update
sudo apt install clang llvm graphviz -y
```
<img width="829" height="315" alt="image" src="https://github.com/user-attachments/assets/639b9c7a-e69c-4ab3-a581-3f77d6315e06" />

Создание рабочей папки:
```
mkdir -p ~/lab7
cd ~/lab7
```
<img width="465" height="68" alt="image" src="https://github.com/user-attachments/assets/71b01133-2847-4b70-b5a4-66e92f4ada1d" />

## Общая часть работы
## Заданный исходный код
```
#include <stdio.h>

int square(int x) {
    return x * x;
}

int main() {
    int a = 5;
    int b = square(a);
    printf("%d\n", b);
    return 0;
}
```

## Получение AST
Команда:
```
clang -Xclang -ast-dump -fsyntax-only main.c
```
Результат:
<img width="1855" height="820" alt="image" src="https://github.com/user-attachments/assets/abb41683-0a17-4e03-bde7-a6c7efdddaa4" />

## Генерация LLVM IR без оптимизаций (-O0)
Команда: 
```
clang -O0 -S -emit-llvm main.c -o main_O0.ll
```
Результат:
<img width="1615" height="846" alt="image" src="https://github.com/user-attachments/assets/291ff4d1-d3df-42aa-ac88-36ec5bb1a552" />

## Генерация LLVM IR с оптимизацией (-O2)
Команда:
```
clang -O2 -S -emit-llvm main.c -o main_O2.ll
```
Резульат:
<img width="1789" height="716" alt="image" src="https://github.com/user-attachments/assets/3296963e-c781-4ea7-8601-b5f85d9a950a" />

## Оптимизация IR через opt
Команда:
```
opt -passes=default<O2> -S main_O0.ll -o main_opt_O2.ll
```

Результат:
<img width="1348" height="685" alt="image" src="https://github.com/user-attachments/assets/4a056ca4-4187-47aa-9344-5aa857c48d9f" />

## Построение CFG 
До оптимизации:
```
opt -passes=dot-cfg -disable-output main_O0.ll
dot -Tpng .main.dot -o main_cfg_O0.png
```

После оптимизации:
```
opt -passes=dot-cfg -disable-output main_O2.ll
dot -Tpng .main.dot -o main_cfg_O2.png
```

CFG для main до оптимизации (-O0):
<img width="781" height="306" alt="image" src="https://github.com/user-attachments/assets/f910ebeb-a83e-4e8b-8449-f789e5082259" />

# Лабораторная работа №7

## Название
Промежуточное представление и локальные оптимизации

## Цель
Построить промежуточное представление (IR) для синтаксической конструкции из ЛР5, реализовать две локальные оптимизации и продемонстрировать их в GUI.

## Вариант задания
Объявление вещественной константы с инициализацией на языке Pascal (продолжение ЛР5).

## Реализованные пункты

### 1. AST
Строится при нажатии **Пуск (F5)** на вкладке **«Семантика и AST»** (как в ЛР5).

### 2. Промежуточное представление (TAC)
Трёхадресный код генерируется из AST объявлений `const`:
- `declare имя, тип`
- `t = -литерал` — унарный минус через временную переменную
- `t = идентификатор` — копирование из другой константы
- `имя = источник` — инициализация

### 3. Локальные оптимизации
| № | Название | Суть |
|---|----------|------|
| 1 | Свёртка констант | `t = -3.14` (uminus) → `t = -3.14` (assign) |
| 2 | Распространение копий | `t1 = a` при известном `a` → `t1 = 1.0`; `b = t1` → `b = 1.0`; удаление неиспользуемых `t` |

### 4. Демонстрация в GUI
Вкладка **«IR и оптимизация»**: входной IR, входной/выходной IR для каждой оптимизации.

## Примеры для демонстрации

**Свёртка констант и удаление временной:**
```
const x: real = -0.5;
```

**Распространение копий:**
```
const a: real = 1.0;
const b: real = a;
```

## Оптимизации для арифметических выражений (F6)
На вкладке **«Тетрады и ПОЛИЗ»** после **F6** для тетрад применяются:
1. **Свёртка констант** — `(+, 2, 3, t1)` → `(=, 5, , t1)`
2. **Алгебраическое упрощение** — `x+0`, `x*1` → `(=, x, , t)`

Пример: `1+2*3`

## Запуск
```
python main.py
```
- **F5** — объявления `const`: **Семантика и AST**, **IR и оптимизация**
- **F6** — выражение: **Тетрады и ПОЛИЗ** (с оптимизациями тетрад)

CFG для main после оптимизации (-O2):

<img width="660" height="147" alt="image" src="https://github.com/user-attachments/assets/4d0d8132-9f8b-47fe-9d2a-7d2db4e4fdc0" />

## Выводы по общей части
1. С помощью Clang можно получить полную структуру AST и IR, а также CGF;
2. LLVM предоставляет гибкие инструменты анализа и оптимизации;
3. Промежуточное представление кода удобно для написания компиляторных трансформаций.


# Индивидуальная часть работы

## Вариант задания
Вещественные константы

## Исходный код
```
const double PI = 3.141592653589793;
int main() {
double r = 2.0;
double area = PI * r * r;
return (int)area;
}
```

## Задания по варианту
1. Получите IR для -O0.
2. Получите IR для -O2. Произошло ли свертывание константы?
3. Примените -constprop, -globalopt, -ipsccp.
4. Сравните CFG.
5. Сделайте вывод о том, когда вещественная константа
вычисляется на этапе компиляции?

## Задание 1: Получите IR для -O0.
Команда: 
```
clang -O0 -S -emit-llvm const.c -o const_O0.ll
```

Результат: 
<img width="1762" height="720" alt="image" src="https://github.com/user-attachments/assets/d48f2945-5ec6-4805-853e-91170e4b6e46" />

## Задание 2: Получите IR для -O2.
Команда:
```
clang -O2 -S -emit-llvm const.c -o const_O2.ll
```
Результат:
<img width="1848" height="626" alt="image" src="https://github.com/user-attachments/assets/5d7deeb5-d244-4a29-a2a5-c60c505bcb7d" />


## Задание 3: Примените -constprop, -globalopt, -ipsccp.

Применение -constprop:
Команда:
```
opt -passes=instcombine -S const.ll -o const_constprop.ll
```
Результат:
<img width="1765" height="741" alt="image" src="https://github.com/user-attachments/assets/402cf97c-5568-46d8-b9b7-0fc5a7f696d2" />

CFG:

<img width="511" height="350" alt="image" src="https://github.com/user-attachments/assets/ff50de37-bd1b-4bb9-bbd5-7f3fffd2d4ef" />

Применение -globalopt:
Команда:
```
opt -passes=globalopt -S const.ll -o const_globalopt.ll
```
Результат:
<img width="1767" height="722" alt="image" src="https://github.com/user-attachments/assets/042d5667-abc2-45fd-a5bd-f34a21aad8a1" />

CFG:

<img width="503" height="344" alt="image" src="https://github.com/user-attachments/assets/71988a5f-9ae7-4902-a355-ec2c1e33f1a8" />

Применение -ipsccp:
Команда:
```
opt -passes=ipsccp -S const.ll -o const_ipsccp.ll
```
Результат:
<img width="1756" height="729" alt="image" src="https://github.com/user-attachments/assets/80dfd2bd-975c-4537-ac96-25cd4ace6cb2" />

CFG:

<img width="503" height="345" alt="image" src="https://github.com/user-attachments/assets/8ade85f5-0d71-41aa-a366-ebeba13bb59a" />

## Сравнение CFG
CFG для -O0:

<img width="517" height="350" alt="image" src="https://github.com/user-attachments/assets/4ae4ba73-9a8e-498e-a2e3-68b2f28744f9" />

CFG для -O2:

<img width="256" height="102" alt="image" src="https://github.com/user-attachments/assets/b8128f28-dd42-470f-989a-610b8e8d551c" />

Сравнение:
CFG для -O0, instcombine, globalopt, ipsccp идентичны:
1. Один базовый блок
2. Содержат alloca, store, load, fmul, fptosi, ret
3. Количество инструкций ~12
3. Арифметическое выражение PI * r * r не вычислено на этапе компиляции

CFG для -O2 радикально отличается:
1. Один базовый блок
2. Содержит только ret i32 12
3. Выражение PI * 2.0 * 2.0 = 12.566... округлено до 12 (при преобразовании в int) и вычислено на этапе компиляции

Вывод: Полное свёртывание констант и удаление лишних инструкций происходит только на уровне оптимизации -O2. Отдельные оптимизации (instcombine, globalopt, ipsccp) не обеспечивают такого результата, так как требуют комплексного взаимодействия нескольких пассов.

## Вывод (когда вещественная константа вычисляется на этапе компиляции)
Вещественная константа вычисляется на этапе компиляции при соблюдении следующих условий:

1. **Все операнды известны статически** — значения определены в коде и не зависят от внешних факторов (ввод пользователя, системные вызовы).

2. **Применён достаточный уровень оптимизации** — `-O2` или выше (отдельные оптимизации `instcombine`, `globalopt`, `ipsccp` не дают полного эффекта).

3. **Компилятор может доказать отсутствие побочных эффектов** — выражение не содержит вызовов функций, изменяющих глобальное состояние.

**Результат проверки:**
- Без оптимизаций (`-O0`): выражение `PI * r * r` → инструкции `alloca`, `load`, `store`, `fmul` (вычисление в рантайме).
- С оптимизацией (`-O2`): выражение `PI * r * r` → `ret i32 12` (вычислено на этапе компиляции).

**Вывод:** Компилятор LLVM выполняет полное свёртывание вещественных констант только при использовании комплексных уровней оптимизации (`-O2`/`-O3`), запускающих последовательность взаимодополняющих пассов.

## Ответы на контрольные вопросы
1. Что такое Clang, и какова его роль в процессе компиляции программ?

Clang - это фронтенд компилятора. Он необходим для работы с исходным текстом программы (C/C++). Он проверяет синтаксис, строит абстрактное синтаксическое дерево (AST) и переводит код в промежуточное представление (LLVM IR).

2. Что представляет собой LLVM и как он используется в современных компиляторах?

LLVM - это модульная инфраструктура для разработки компиляторов, выполняющая роль оптимизатора и бэкенда. Современные компиляторы используют её как конструктор - берут готовые библиотеки LLVM для оптимизации универсального кода и его перевода в машинные инструкции конкретного процессора (x86, ARM).

3. Чем отличается абстрактное синтаксическое дерево (AST) от промежуточного представления LLVM IR?

- AST - это высокоуровневое дерево структуры кода, привязанное к синтаксису языка C/C++.
- LLVM IR - это низкоуровневый линейный ассемблер, полностью независимый от исходного языка программирования и оперирующий базовыми блоками, метками и регистрами.

4. Для чего необходимо промежуточное представление (IR) в процессе компиляции?

IR избавляет от необходимости писать отдельный компилятор под каждую пару «Язык - Процессор». Вместо этого все языки переводятся в один общий IR. Оптимизации пишутся один раз для IR, и они автоматически работают для любого входного языка и любого целевого процессора.

5. Что делает инструкция alloca в LLVM IR, и зачем она используется в функциях?

Инструкция alloca выделяет память на стеке текущего кадра функции для локальной переменной. Она используется, чтобы гарантировать переменной её физический адрес в памяти для корректного выполнения операций чтения (load) и записи (store).

6. Зачем нужна оптимизация кода в компиляторе, и какие основные цели она преследует?

Оптимизация нужна для улучшения характеристик программы без изменения логики её работы. Она позволяет увеличить скорость выполнения, уменьшить размер бинарного файла и снизить потребление оперативной памяти или энергии.

7. Что такое SSA-форма и почему она важна при оптимизации программ?

SSA (Static Single Assignment) - это форма кода, где каждому виртуальному регистру значение присваивается ровно один раз. Если переменная в C++ меняется, в IR создаются новые регистры (%x1, %x2). При ветвлениях используется специальная ϕ-функция (Phi-node) для выбора нужного значения. SSA важна, так как она убирает ложные зависимости между инструкциями, делая оптимизации простыми и быстрыми.

8. Что такое граф потока управления (CFG) и как он помогает анализировать поведение программы?

CFG - это ориентированный граф функции, где:

- Вершины - базовые блоки (куски кода без ветвления внутри, выполняющиеся строго линейно).
- Ребра - стрелки переходов (br, switch) между ними.
Он помогает компилятору видеть логику переходов, находить циклы, оптимизировать условия и удалять недостижимый код.

9. Как устроено представление арифметических операций в LLVM IR (например, умножение, сложение)?

Они представлены в строго типизированной трехадресной форме - операция принимает два операнда фиксированного типа и записывает результат в новый регистр. Также могут быть флаги (например, nsw - отсутствие знакового переполнения). Пример: %res = add nsw i32 %0, %1 (сложение двух 32-битных целых чисел).

10. Почему функции в LLVM IR обычно представляют собой отдельные единицы анализа и оптимизации?

Функции обладают локальностью: у них изолированные переменные, четкие аргументы и границы. Анализировать всю программу сразу слишком долго и ресурсоемко, а анализ отдельных функций позволяет компилятору работать быстро и эффективно.

11. Что происходит с функцией в LLVM IR, если она вызывается один раз и очень короткая?

При включении оптимизаций (например, -O2) оптимизатор применяет инлайнинг (Inlining). Тело этой короткой функции встраивается прямо в место её вызова, а сама функция удаляется. Это убирает накладные расходы процессора на прыжки по коду (call/ret) и сохранение регистров.

12. Какие преимущества даёт использование IR и CFG для автоматических оптимизаций по сравнению с анализом исходного текста на C?

В IR нет синтаксического разнообразия: любые циклы (for, while, do-while) в IR превращаются в одинаковые базовые блоки и переходы br. Действует строгое правило SSA, благодаря которому легко отслеживать, откуда взялось значение в регистре. Граф CFG дает четкую математическую модель связей в коде, которую невозможно построить простым анализом текста на С из-за сложных областей видимости и макросов.

## Дополнительное задание
## Оптимизация 1:
### Блок-схема
<img width="406" height="616" alt="image" src="https://github.com/user-attachments/assets/63b9e48a-8085-40bd-a199-0d6f6e103e28" />

### Работа программы:
<img width="1351" height="745" alt="image" src="https://github.com/user-attachments/assets/d45b914c-98a9-4d8f-bd50-a07168579fb1" />

## Оптимизация 2:
### Блок-схема
<img width="381" height="646" alt="image" src="https://github.com/user-attachments/assets/c12ff6ed-74f4-47d4-b68a-1229b3e9df12" />

### Работа программы
<img width="1338" height="788" alt="image" src="https://github.com/user-attachments/assets/1f007b97-fd2e-4903-a6ce-1bf6c6a02983" />










                                                








